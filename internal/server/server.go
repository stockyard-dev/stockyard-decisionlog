package server
import("encoding/json";"net/http";"github.com/stockyard-dev/stockyard-decisionlog/internal/store")
...
func(s *Server)routes(){
    s.mux.HandleFunc("GET /health",func(w http.ResponseWriter,r *http.Request){writeJSON(w,200,map[string]string{"status":"ok","service":"stockyard-decisionlog"})})
    s.mux.HandleFunc("GET /api/stats",s.handleOverview)
    s.mux.HandleFunc("GET /api/decisions",s.handleList)
    s.mux.HandleFunc("POST /api/decisions",s.handleCreate)
    s.mux.HandleFunc("PUT /api/decisions/{id}",s.handleUpdate)
    s.mux.HandleFunc("DELETE /api/decisions/{id}",s.handleDelete)
