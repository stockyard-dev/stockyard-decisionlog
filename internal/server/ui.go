...
if(d.outcome)h+='<div class="dec-outcome"><div class="dec-outcome-label">Outcome</div>'+esc(d.outcome)+'</div>';
...
function formHTML(dec){var i=dec||{title:'',context:'',options:'',outcome:'',decided_by:'',status:'pending',decided_at:''};var isEdit=!!dec;
...
h+='<div class="fr"><label>Outcome</label><textarea id="f-outcome" rows="2" placeholder="What was decided?">'+esc(i.outcome)+'</textarea></div>';
...
['pending','decided','revisiting'].forEach(function(s){h+='<option value="'+s+'"'+(i.status===s?' selected':'')+'>'+s.charAt(0).toUpperCase()+s.slice(1)+'</option>';});
h+='</select></div></div>';
h+='<div class="fr"><label>Date Decided</label><input id="f-date" type="date" value="'+esc(i.decided_at)+'"></div>';
...
var body={title:title,context:document.getElementById('f-ctx').value.trim(),options:document.getElementById('f-opts').value.trim(),outcome:document.getElementById('f-outcome').value.trim(),decided_by:document.getElementById('f-by').value.trim(),status:document.getElementById('f-status').value,decided_at:document.getElementById('f-date').value};
